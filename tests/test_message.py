...
def test_message_creation(message_login):
	assert message_login

def test_message_render(message_login):
	data = message_login.render()
	assert len(data) == 1+16+2+len(message_login.UID)

def test_message_parse(message_login):
	data = message_login.render()
...
def test_message_authentication(authentication_state, message_data_short):
